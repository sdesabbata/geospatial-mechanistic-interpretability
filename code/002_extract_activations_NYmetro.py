...
df = pd.read_csv('storage/geonames_NYmetro.csv')
df = df.loc[(df['feature class'] == 'P') & (df['population'] > 0)]
df['admin1 code'] = df['admin1 code'].astype(str)
df = df[df['admin1 code']!= 'nan']
# print(np.unique(df['admin1 code']))
convdict = {'CT':'Connecticut',
            'NJ':'New Jersey',
            'NY':'New York',
            'PA':'Pennsylvania'
}
...
def get_prompt(x):
    return f"{x['name']}, {convdict[x['admin1 code']]}"
...
# Save activations
df.to_pickle('storage/activations_NYmetro.pkl')
